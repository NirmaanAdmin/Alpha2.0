...
try {
    ...
    // Insert only for TODAY, only if not already present for that staff_id+date.
    // value = shift duration in HOURS (handles overnight shifts).
    $sql = "
        ...
        FROM (
            ...
            WHERE cio.type_check = 1
              AND DATE(cio.date) = :run_date          -- ONLY today's rows
            GROUP BY cio.staff_id, DATE(cio.date)
            HAVING COUNT(*) = 1                        -- keep your original rule
        ) AS t
        LEFT JOIN (
            -- latest shift per staff_id; adjust if your table has a date to join on
    ...
    $stmt->execute([':run_date' => $today]);

    echo "Inserted " . $stmt->rowCount() . " row(s) for " . $today;
