...
function update_all_sale_estimate_fields_activity_log($id, $new_data)
{
    $CI = &get_instance();
    ...
    $field_map = [
        ...
        'date' => _l('estimate_add_edit_date'),
        'expirydate' => _l('estimate_add_edit_expirydate'),
        'clientnote' => _l('estimate_add_edit_client_note'),
    ...
    foreach ($changes as $field => $dummy) {
        ...
        update_sale_estimate_activity_log($id, $field_map[$field], $old_value, $new_value);
    }
    return true;
}

function update_sale_estimate_activity_log($id, $field, $old_value, $new_value)
{
    $CI = &get_instance();
    if(!empty($id)) {
        $CI->db->where('id', $id);
        ...
        if(!empty($estimates)) {
            ...
            $description = "".$field." field has been updated from <b>".$old_value."</b> to <b>".$new_value."</b> in estimate <b>".format_estimate_number($estimates->id)."</b>.";
            $CI->db->insert(db_prefix() . 'module_activity_log', [
                'module_name' => 'estimates',
                'rel_id' => $estimates->id,
                'description' => $description,
