...
function update_all_inv_payment_fields_activity_log($id, $new_data)
{
    $CI = &get_instance();
    $CI->load->model('currencies_model');
    $base_currency = $CI->currencies_model->get_base_currency();
    ...
    foreach ($changes as $field => $dummy) {
        ...
        update_inv_payment_activity_log($id, $field_map[$field], $old_value, $new_value);
    }
    return true;
}

function update_inv_payment_activity_log($id, $field, $old_value, $new_value)
{
    $CI = &get_instance();
    if(!empty($id)) {
        $CI->db->where('id', $id);
        ...
        if(!empty($invoicepaymentrecords)) {
            ...
            if(!empty($invoices)) {
                $description = "".$field." field has been updated from <b>".$old_value."</b> to <b>".$new_value."</b> for the payment of invoice <b>".format_invoice_number($invoices->id)."</b>.";
                $CI->db->insert(db_prefix() . 'module_activity_log', [
                    'module_name' => 'inv_payment',
                    'rel_id' => $invoices->id,
                    'description' => $description,
