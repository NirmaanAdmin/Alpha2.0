(function(){
	...
	initDataTable('.table-accessories', admin_url + 'fixed_equipment/accessories_table', [0, 10],[0, 10], fnServerParams, [1, 'desc']);
	...
	$(document).on("change","#for_sell, #for_rent",function() {
		...
		appValidateForm($('#accessories-form'), data_validate)
	});
 ...
 */
 function add(){
 	...
 	$('#add_new_accessories .add-title').removeClass('hide');
 	$('#add_new_accessories .edit-title').addClass('hide');
 	$('#add_new_accessories input[name="id"]').val('');
 	$('#add_new_accessories input[type="text"]').val('');
 	$('#add_new_accessories input[type="number"]').val('');
 	$('#add_new_accessories select').val('').change();
 	$('#add_new_accessories textarea').val('');
 	$('#add_new_accessories input[type="checkbox"]').prop('checked', false).change();
 	$('#ic_pv_file').remove();
 }
 ...
 function bulk_delete(){
	...
	if(print_id != ''){
		if(confirm($('input[name="are_you_sure_you_want_to_delete_these_items"]').val()) == true){
			window.location.href = admin_url+"fixed_equipment/delete_all_item/"+encodeURIComponent(print_id)+"?type=accessory";
		}
	}
	else{
