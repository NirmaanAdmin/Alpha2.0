...
$projects = $this->ci->input->post('projects');
if (isset($projects)) {
    $where_projects = '';
    foreach ($projects as $t) {
        if ($t != '') {
            if ($where_projects == '') {
                $where_projects .= ' AND (' . db_prefix() . 'pur_invoices.project_id = "' . $t . '"';
            } else {
                $where_projects .= ' or ' . db_prefix() . 'pur_invoices.project_id = "' . $t . '"';
            }
        }
    }
    if ($where_projects != '') {
...
$payment_status = $this->ci->input->post('status');
...
if (!empty($payment_status) && is_array($payment_status)) {
    ...
    foreach ($payment_status as $status) {
        if ($status !== '') {
            $statuses[] = $this->ci->db->escape($status);
        }
    }
